...
export default function InvestmentCalculator() {
  ...
  /* ── Nekonečná renta — vlastní výnosový slider ── */
  const [rentaRate, setRentaRate] = useState(5);

  /* ── Výpočty hlavní kalkulačky ── */
  ...
  const profitPct = totalInvested > 0 ? Math.round((profit / totalInvested) * 100) : 0;

  /* ── Výpočty nekonečné renty ── */
  const annualRenta = finalValue * (rentaRate / 100);
  const monthlyRenta = annualRenta / 12;
  ...
  return (
    <>
      {/* ════════════════════════════════════════
          SEKCE 1 — Kalkulačka složeného úročení
      ════════════════════════════════════════ */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-start">
            ...
            <div className="space-y-7">
              ...
              <Slider label="Investiční horizont" value={years} min={1} max={30} step={1}
                format={(v) => `${v} ${v === 1 ? "rok" : v < 5 ? "roky" : "let"}`} onChange={setYears} />
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="bg-white rounded-2xl p-5 border border-[#e8e5e2]">
                  ...
                  <p className="text-[#c5a889] text-xs mt-0.5">+{profitPct} %</p>
                </div>
              </div>
      ...
      {/* ════════════════════════════════════════
          SEKCE 2 — Nekonečná renta
      ════════════════════════════════════════ */}
      <section className="bg-white py-24 lg:py-32 border-t border-[#e8e5e2]">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="mb-12 lg:mb-16 max-w-2xl">
            <p className="text-[#97724f] text-sm font-heading font-semibold uppercase tracking-[0.2em] mb-4">
              Nekonečná renta
            ...
            <p className="text-[#6b6b6b] text-base leading-relaxed">
              Nekonečná renta ukazuje, kolik lze trvale vybírat z vašeho portfolia, aniž by se jeho hodnota snižovala.
              Vychází z konečné hodnoty portfolia po {years} {years === 1 ? "roce" : years < 5 ? "letech" : "letech"} ({formatCZK(finalValue)}).
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            ...
            <div className="space-y-8">
              {/* Přehled z kalkulačky */}
              <div className="bg-[#f6f6f6] rounded-2xl p-6 border border-[#e8e5e2]">
                <p className="text-xs text-[#6b6b6b] mb-3 uppercase tracking-wider font-semibold">Vychází z výsledku kalkulačky</p>
                <div className="flex items-baseline justify-between">
                  <span className="text-sm text-dark">Hodnota portfolia po {years} letech</span>
                  <span className="font-heading font-700 text-[#97724f] text-lg">{formatCZK(finalValue)}</span>
                </div>
              </div>

              <Slider
                label="Výnos z portfolia"
                hint="(p.a.) — nezávislý na modelovém výnosu"
                value={rentaRate}
                min={1}
                max={12}
                step={0.5}
              ...
              <p className="text-[#9b9b9b] text-xs leading-relaxed">
                Výnos z portfolia pro rentu je záměrně oddělený od modelového výnosu investice.
                Pro konzervativní plánování doporučujeme použít nižší hodnotu (3–5 %).
              </p>
            </div>
            ...
            <div className="space-y-4">
              <div className="bg-[#97724f] rounded-3xl p-8 lg:p-10 text-center">
                ...
                <p className="font-heading font-800 text-white text-4xl lg:text-5xl mb-1">
                  {formatCZK(monthlyRenta)}
                </p>
                <p className="text-white/50 text-xs">každý měsíc, navždy</p>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div className="bg-[#f6f6f6] rounded-2xl p-6 border border-[#e8e5e2] text-center">
                  <p className="text-[#6b6b6b] text-xs mb-2">Roční renta</p>
                  <p className="font-heading font-700 text-dark text-base">{formatCZK(annualRenta)}</p>
                </div>
                <div className="bg-[#f6f6f6] rounded-2xl p-6 border border-[#e8e5e2] text-center">
                  <p className="text-[#6b6b6b] text-xs mb-2">Zachováno jmění</p>
                  <p className="font-heading font-700 text-dark text-base">{formatCZK(finalValue)}</p>
                </div>
              </div>

              <p className="text-[#9b9b9b] text-xs leading-relaxed text-center">
                Princip nekonečné renty: z portfolia vybíráte pouze výnosy, kapitál zůstává nedotčen a pracuje dál.
              </p>
            </div>
          </div>
        </div>
